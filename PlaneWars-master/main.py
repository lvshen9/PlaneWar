...
pygame.display.set_caption('一起来打飞机————by lvshen')
...
shootMusic = pygame.mixer.music.load('resources/sound/bullet.mp3')

#升级音效，同上
#level_up_music = pygame.mixer.Sound('resources/sound/get_bomb.mp3')
level_up_music = pygame.mixer.music.load('resources/sound/get_bomb.mp3')
...
enemy_shoot_sound = pygame.mixer.music.load('resources/sound/enemy3_down.mp3')
...
while RUN:
    ...
    if not player.is_hit:
        ...
        player.img_index = int(SHOOT_PC / 248) + 1

    else:
        player.img_index = int(player_shoot / 248)
        screen.blit(player.image[player.img_index], player.rect)
        player_shoot += 30
        if player_shoot > 495:
            RUN = False
    ...
    for enemy_shoots in enemies_shoot:
        ...
        if enemy_shoots.shoot_index > 70:
            ...
            score += 100
            continue
        screen.blit(enemy_shoots.shoot_imgs[int(enemy_shoots.shoot_index / 20)], enemy_shoots.rect)
    ...
    if score == 100 * (level ** 2 + level):
        level += 1
        pygame.mixer.music.load('resources/sound/get_bomb.mp3')
        pygame.mixer.music.play()
        if level != 10:
            enemy_add -= 20
    ...
    for enemy_shoot in enemies_is_shoot:
        pygame.mixer.music.load('resources/sound/enemy3_down.mp3')
        pygame.mixer.music.play()
